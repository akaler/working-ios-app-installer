...
-(void) disconnect:(NSString *)addr didResponse:(ResponseBlock)block
{
    dispatch_async(_queue, ^{
        ...
        if (addr)
            ...
            snprintf(buf, sizeof buf, "host:capability:");//:%s", addr.UTF8String);
        else
    ...
    printf("Printing after dispatch asych disconnect");
}
...
{
    printf("Line 249: UNINSTALL APK PRINT OUTSIDE OF DISPATCH_ASYNCH");
    dispatch_async(_queue, ^{
        //[self pm:[NSString stringWithFormat:@"uninstall %@", packageName] toResponse:block];
        
        int fd = adb_connect("capability:");
        printf("thefd is capability: %d",fd);
    });
    printf("Line 256: UNINSTALL APK PRINT OUTSIDE OF DISPATCH_ASYNCH");
}
